...
## `implemented` is false for all 180 today (slices 15-20 build the trigger
## engine and wire mechanics one at a time); an inert artefact must never be
## offered, so only ARTEFACT_EFFECTS below is ever rolled, sold or granted.
