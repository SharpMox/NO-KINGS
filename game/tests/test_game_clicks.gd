...
func _init() -> void:
	...
	# NO-113: merge_panel (modals.gd) is a PRESET_FULL_RECT, move_to_front()'d
	# PanelContainer — default MOUSE_FILTER_STOP — that the "Merge" confirm
	# above only hides at the END of a Tuning.MERGE_ANIM_S (0.35s) tween
	# (modals.gd's _play_merge_animation), not on the click itself. Only one
	# process_frame separates that click from this one, so if the animation
	# is still running, this click lands on merge_panel, never on pass_button.
