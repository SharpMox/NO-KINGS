...
- **Slot counts become data, not literals** (02): `ROWS` grows a modifier path so an Artefact can add slots (Chocolate Key Cake +2 Item, Alleged Weather Balloon +1 Item, Sub-Antarctic Visa +1 Artefact). No artefact in the prototype uses it yet — the seam exists because the stock shape is now defined as base + modifiers, not because a caller is waiting.
