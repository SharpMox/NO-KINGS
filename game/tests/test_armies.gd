extends SceneTree
## The Army framework (issue 67): catalog sanity, kit application on a
...
## out by name. Run headless:  godot --headless --path game -s tests/test_armies.gd
...
## only way to exercise the Army kit application itself (Gold/Items),
...
func _init() -> void:
	# --- catalog sanity ---
	check(Armies.CATALOG.size() == 6, "six Armies: issue 67's three seeds + issue 68's three more")
	for id in Tuning.ARMIES:
		check(Armies.CATALOG.has(id), "%s: every Army id has a Army entry (load-bearing save key)" % id)
	check(Armies.display_name("Crown") == "The Muster",
		"Crown's DISPLAY name is The Muster (\"The Levy\" vetoed) — the save id stays \"Crown\"")
	check(Armies.display_name("Wild Hunt") == "Wild Hunt" and Armies.display_name("Old Guard") == "Old Guard",
	...
	await process_frame
	check(muster.gold == Tuning.ARMY_BASELINE_GOLD, "The Muster: baseline Gold")
	...
	await process_frame
	check(hunt.gold == Tuning.ARMY_BASELINE_GOLD / 2, "Wild Hunt: ~half Gold")
	...
	await process_frame
	check(guard.gold == Tuning.ARMY_BASELINE_GOLD / 2, "Old Guard: ~half Gold")
	...
	# — Armies.blood_in_the_air's own independent check at the same
	...
	await process_frame
	gate.actions_left = 0
	check(not gate._army_ability_available(), "Ability gate: unavailable at 0 actions_left")
	gate.actions_left = 2 # a spare action left over, so the spend below lands
		# at 1, not 0 — 0 would auto-pass the Turn (_army_ability_confirmed's
	...
	check(gate._army_ability_available(), "Ability gate: available at >=1 actions_left")
	gate._army_ability_confirmed()
	check(gate.army_ability_used_this_wave and gate.actions_left == 1,
		"Ability activation spends exactly 1 Action (2 -> 1) and marks the Wave flag")
	check(not gate._army_ability_available(),
		"Ability gate: unavailable again once used this Wave (even with actions_left topped up)")
	gate.actions_left = 2
	check(not gate._army_ability_available(), "(still) once-per-Wave, not once-per-Turn")
	WaveLogic.queue(gate, gate.wave + 1)
	check(not gate.army_ability_used_this_wave, "Ability re-arms on Wave clear")
	gate.queue_free()
	await process_frame
	...
	full_cap._army_ability_confirmed()
	...
	hounds._army_ability_confirmed() # spends 1: actions_left -> 2
	...
	check(banners._army_ability_available(), "(setup) Call the Banners is available")
	banners._begin_army_targeting()
	check(banners.army_targeting, "Call the Banners begins targeting (no confirm modal)")
	var target: Variant = banners.stock[1] # the stateful ferz entry
	banners._army_target_stock(target, false)
	...
	cancel._begin_army_targeting()
	cancel._begin_army_targeting() # tap the (virtual) chip again
	check(not cancel.army_targeting and cancel.stock.size() == 1 and cancel.actions_left == 1
			and not cancel.army_ability_used_this_wave,
		"Call the Banners: cancelling FROM TARGETING costs nothing — no duplicate, no charge")
	cancel.queue_free()
	await process_frame

	# ================= issue 68: three more Armies =================

	# --- kit application on a fresh boot ---
	var syndicate := _boot_fresh("Syndicate")
	await process_frame
	check(syndicate.gold == Tuning.ARMY_BASELINE_GOLD * 3, "The Syndicate: TRIPLE baseline Gold")
	...
	await process_frame
	check(cult.gold == Tuning.ARMY_BASELINE_GOLD, "The Cult: baseline Gold")
	...
	await process_frame
	check(horde.gold == Tuning.ARMY_BASELINE_GOLD, "The Horde: baseline Gold")
	...
	check(Shop.price(no_rates, piece_slot) == syn_pawn_value
		...
		"(sanity) Insider Rates is scoped to the Syndicate — a non-Syndicate Army pays/receives base rates")
	no_rates.queue_free()
	await process_frame
	...
	strip._begin_army_board_targeting()
	check(strip.army_board_targeting and strip.army_board_targets.has(Vector2i(3, 3))
	...
	strip._army_board_target_click(Vector2i(3, 3))
	...
	check(strip.actions_left == 1 and strip.army_ability_used_this_wave,
	...
	check(not king_only._army_ability_available(),
	...
	cancel_takeover._begin_army_board_targeting()
	cancel_takeover._begin_army_board_targeting() # tap the (virtual) chip again
	check(not cancel_takeover.army_board_targeting and cancel_takeover.board.has(Vector2i(7, 10))
			and cancel_takeover.gold == gold_before2 and cancel_takeover.actions_left == 1
			and not cancel_takeover.army_ability_used_this_wave,
	...
	ritual._begin_army_board_targeting()
	check(ritual.army_board_targeting and ritual.army_board_targets == [Vector2i(2, 2)]
			and ritual.hud.drawer_open == "",
		"Ritual stages targeting on the BOARD — only the player's own piece is a valid target")
	ritual._army_board_target_click(Vector2i(2, 2))
	check(BuffLogic.catalogued_count(ritual.board[Vector2i(2, 2)]) == 1,
		"Ritual: the target gains exactly one random Buff")
	check(ritual.actions_left == 1 and ritual.army_ability_used_this_wave,
	...
	cancel_ritual._begin_army_board_targeting()
	cancel_ritual._begin_army_board_targeting() # tap the (virtual) chip again
	check(not cancel_ritual.army_board_targeting
			and BuffLogic.catalogued_count(cancel_ritual.board[Vector2i(2, 2)]) == 0
			and cancel_ritual.actions_left == 1 and not cancel_ritual.army_ability_used_this_wave,
	...
	check(crown_deploy.gold == gold_before5 - pawn_cost,
		"(sanity) Endless Ranks is scoped to the Horde — a non-Horde Army still pays for a pawn deploy")
	...
	consc._army_ability_confirmed()
	check(consc.stock.size() == 2 and consc.stock.count("pawn") == 2,
		"Conscription: adds exactly 2 pawns to Stock, as bare ids")
	check(consc.actions_left == 1 and consc.army_ability_used_this_wave,
		"Conscription spends its 1 Action and the once-per-Wave flag")
	consc.queue_free()
	await process_frame

	# --- merged from the old tests/test_armies.gd (issue 76) ---
	# Army COMPOSITION rules: the original three get the strict
	# shape/balance check; issue 68's three are deliberate outliers by that
	# rule's own standard, so they only get the universal 'every id is a real
	# piece' check. Kept verbatim rather than dropped — renaming this file
	# over the old one would have silently destroyed these.
