...
deliberately one line per artefact with no image slot. 38 of the 180 are painted, and the
page emits an `<img>` **only** for those: `data/artefact-art.js` maps artefact `name` to
file path and is generated by `tools/export-artefact-art.mjs`, which stats each file
before listing it. The other 142 get no image element at all — not a broken one, not a
placeholder. Never learn a file is missing by watching a request fail; 142 missing files
would be 142 console 404s and the verification rule below is *zero* console errors.
...
- `node tools/export-artefact-art.mjs` -> `data/artefact-art.js` (which artefacts have art)
