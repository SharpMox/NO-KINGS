...
- `PixelOperator-Bold.ttf` draws the banners. `PixelOperator.ttf` (regular, 32 px) draws the kill feed (NO-239, hud.gd).
...
- Design grid: 16 px for both weights (1600 units/em, 100 units per pixel; odd multiples occur,
  so 24 px would split font pixels 1.5 screen px wide). Draw it at 16 or 32 so every pixel
  stays whole.
