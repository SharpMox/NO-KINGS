...
- **Worktree-first still applies.** All editor mutations happen against the worktree's project copy, not main. Per `[[execute]]`: never edit on main.
- **Stale index after merges.** Godot AI doesn't (currently) re-index on `git merge`. After `noodle worktree merge`, re-open the project in the integration branch if you intend to verify visually.
...
- **Pre-bootstrap state** (current): no `project.godot` yet. Tools unavailable until the Godot mobile project is initialized (`todos.md` item #1).
...
- `[[execute]]` — implementation methodology (worktree-first, scope discipline).
