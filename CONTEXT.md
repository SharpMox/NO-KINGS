...
A Godot 4 mobile game built by AI agents. **Live work lives in Linear** (workspace `no-kings`, team `NO`, ids `NO-<n>`), adopted 2026-09-06; `.scratch/gdd-gaps/` (a PRD plus one file per closed slice) is the read-only archive of everything shipped before that — only the live items were migrated. The Notion GDD is the design source of truth for the catalogs. This line read *"not Linear, which was never actually used"* until 2026-09-06 — accurate for the whole archive, and no longer accurate for new work.
...
**Drawer**:
A panel holding what does not fit on the main view — Stock and Inventory — that opens next to the button that opens it rather than in one fixed place, at a fixed height that never resizes with its contents. The Stock Drawer opens downward from the Header's Stock button, over the top of the board; the Inventory Drawer opens upward from the Deck's Inventory button, over the bottom of the board, as one scrolling column — an Items grid, then an Artefacts grid where an activatable Artefact carries a ⚡ marker (NO-85; there is no longer a separate Activate section). Placement happens by dragging a piece out of the open Stock Drawer, which is what makes the Drawer the place Stock is *operated* while the Header's Stock button only reports it. Both Drawers close while the player targets a board tile and reopen on cancel, or after use only if something in them is still usable.
